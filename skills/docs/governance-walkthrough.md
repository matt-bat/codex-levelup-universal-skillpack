...
Those paths are governed because they can change agent behavior, validation gates, or release policy.
...
Use `pass` only when the related evidence exists. Use `waived` only with a concrete reason in artifact notes.
